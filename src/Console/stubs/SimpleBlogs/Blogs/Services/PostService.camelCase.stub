...
class PostService 
{
    ...
    protected function uploadImageWithResize($file, $paramFileName = null, $post = null)
    {
        ...
        if($file) {  
            ...
            if(!Storage::exists(config('constants.path.posts.normals'))) {
                Storage::makeDirectory(config('constants.path.posts.normals'));
            }

            if(!Storage::exists(config('constants.path.posts.thumbnails'))) {
                Storage::makeDirectory(config('constants.path.posts.thumbnails'));
            }

            $uniqueName = time() . '_' . $paramFileName .  '.' . $file->extension();
        
            $destinationPath = storage_path(config('constants.path.storage_app_local') . '/' . config('constants.path.posts.thumbnails'));
            ...
            $destinationPath = storage_path(config('constants.path.storage_app_local') . '/' . config('constants.path.posts.normals'));
