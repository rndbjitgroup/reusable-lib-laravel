    ...
    protected function uploadImageWithResize($file, $paramFileName = null, $profile = null)
    {
        ...
        if($file) {  
            if($profile && $profile->profilePhotoPath) { 
                Storage::delete($profile->profilePhotoPath);
            } 

            if(!Storage::exists(config('constants.path.profile'))) {
                Storage::makeDirectory(config('constants.path.profile'));
            }

            $uniqueName = time() . '_' . $paramFileName .  '.' . $file->extension();
        
            $destinationPath = storage_path(config('constants.path.storage_app_local') . '/' . config('constants.path.profile'));
