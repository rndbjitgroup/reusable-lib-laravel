...
class ProfileService 
{
    ...
    {
        $rtrData['profile_photo_path'] = null; 
        if($file) {  
            if($profile) { 
                Storage::delete(config('constants.path.storage_public') . '/' . config('constants.path.profile') . '/' . $profile->profile_photo_path);
            } 
            ...
            $rtrData['profile_photo_path'] = config('constants.path.profile') . '/' . $uniqueName;
    ...
    public function update($request)
    {
        ...
        if ($request->has('photo')) {
            ...
            $request->merge([ 
                'profile_photo_path' => $data['profile_photo_path'] ? $data['profile_photo_path'] : null 
            ]); 
        }
